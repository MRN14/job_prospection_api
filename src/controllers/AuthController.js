...
export const register = async (req, res) => {
    ...
    try {
        ...
        if (!email || !password || !firstName || !lastName) {
            return res.status(400).json({ message: 'Error when user created' })
        }
...
export const logout = async (req, res) => {
    ...
    if (!authHeader) {
        ...
        return res.status(400).json({ message: "Error during disconnection " });
    }
    // return a success message, the client should handle token deletion on their side
